...
const MacOSServerMonitoring = () => {
  ...
  const loadServers = async () => {
    ...
    try {
      ...
      // Filter macOS servers
      const macServers = data.filter(server => 
        server.os && (
          server.os.toLowerCase().includes('mac') ||
          server.os.toLowerCase().includes('darwin') ||
          server.os.toLowerCase().includes('osx')
        )
      );

      const enhancedServers = macServers.map(server => {
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <AppleIcon sx={{ fontSize: 40, color: '#000000' }} />
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={12} sm={6} md={3}>
          <Card sx={{ borderLeft: '4px solid #000000' }}>
            <CardContent>
      ...
      {loading ? (
        ...
        <Alert severity="info">
          No macOS servers found in <strong>{environment.toUpperCase()}</strong> environment.
          {environment === 'uat' && (
            <Box sx={{ mt: 1 }}>
            ...
            <Paper key={server.id} sx={{ p: 3, mb: 3 }}>
              ...
              {metricsData[server.id] && metricsData[server.id].length > 0 ? (
                <Grid container spacing={3}>
                  <Grid item xs={12} md={6}>
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      ...
                      >
                        ...
                        <Area type="monotone" dataKey="cpu" stroke="#2196F3" fill={`url(#cpuGradient-${server.id})`} name="CPU" />
                  ...
                  <Grid item xs={12} md={6}>
                    <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 'bold' }}>Memory Usage (%)</Typography>
                    <ResponsiveContainer width="100%" height={250}>
                      <AreaChart
                        data={metricsData[server.id]}
                        onMouseDown={(e) => e && setRefAreaLeft(e.activeLabel)}
                      ...
                      >
                        ...
                        <CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" />
                                                ...
                                                />                        <YAxis domain={[0, 100]} stroke="#666" />
                        <Tooltip content={<CustomTooltip />} />
                        <Area type="monotone" dataKey="memory" stroke="#4CAF50" fill={`url(#memoryGradient-${server.id})`} name="Memory" />
                        {refAreaLeft && refAreaRight ? (
                          <ReferenceArea x1={refAreaLeft} x2={refAreaRight} strokeOpacity={0.3} />
                        ) : null}
                      </AreaChart>
                  ...
                  <Grid item xs={12} md={6}>
                    <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 'bold' }}>Disk Usage (%)</Typography>
                    <ResponsiveContainer width="100%" height={250}>
                      <AreaChart
                        data={metricsData[server.id]}
                        onMouseDown={(e) => e && setRefAreaLeft(e.activeLabel)}
                      ...
                      >
                        ...
                        <CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" />
                                                ...
                                                />                        <YAxis domain={[0, 100]} stroke="#666" />
                        <Tooltip content={<CustomTooltip />} />
                        <Area type="monotone" dataKey="disk" stroke="#FF9800" fill={`url(#diskGradient-${server.id})`} name="Disk" />
                        {refAreaLeft && refAreaRight ? (
                          <ReferenceArea x1={refAreaLeft} x2={refAreaRight} strokeOpacity={0.3} />
                        ) : null}
                      </AreaChart>
                  ...
                  <Grid item xs={12} md={6}>
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      ...
                      >
                        ...
                        <Line type="monotone" dataKey="memory" stroke="#4CAF50" name="Memory" strokeWidth={2} dot={false} />
                        <Line type="monotone" dataKey="disk" stroke="#FF9800" name="Disk" strokeWidth={2} dot={false} />
                        <Line type="monotone" dataKey="network_bandwidth" stroke="#9C27B0" name="Network" strokeWidth={2} dot={false} />
                        {refAreaLeft && refAreaRight ? (
                          <ReferenceArea x1={refAreaLeft} x2={refAreaRight} strokeOpacity={0.3} />
                        ) : null}
                      </LineChart>
...
export default MacOSServerMonitoring;
