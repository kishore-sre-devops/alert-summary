...
const LinuxServerMonitoring = () => {
  ...
  const loadServers = async () => {
    ...
    try {
      ...
      const linuxServers = data.filter(server => 
        server.os && server.os.toLowerCase().includes('linux')
      );

      // Add calculated status
      const enhancedServers = linuxServers.map(server => {
        ...
        if (server.status === 'online' && !server.is_inactive) {
          ...
          if (cpu >= cpuT.error || memory >= memT.error || disk >= diskT.error) {
            calculatedStatus = 'warning'; // Note: This UI uses 'warning' as the high-alert color/status
  ...
  const loadServerMetrics = async (serverId) => {
    try {
      ...
      // Fetch metrics for cpu, memory, disk, network, uptime
      const metricNames = [
  ...
  useEffect(() => {
    ...
    // Only reload if not custom, or if custom and both dates are set
    if (timeRange !== 'custom' || (customStartDate && customEndDate)) {
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <StorageIcon sx={{ fontSize: 40, color: '#FF9800' }} />
      ...
      {loading ? (
        ...
        <Alert severity="info">
          No Linux servers found in <strong>{environment.toUpperCase()}</strong> environment.
          {environment === 'uat' && (
            <Box sx={{ mt: 1 }}>
        ...
        <>
                ...
                <TableBody>
                  {servers.map((server) => (
                    <TableRow key={server.id} hover>
                      ...
                      <TableCell>
                        <StatusBadge status={server.calculatedStatus} />
                      </TableCell>
          ...
          {/* Metrics Charts for Each Server */}
          {servers.map((server) => (
            <Paper key={server.id} sx={{ p: 3, mb: 3 }}>
              ...
              {metricsData[server.id] && metricsData[server.id].length > 0 ? (
                <Grid container spacing={3}>
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      ...
                      >
                        ...
                        <Area 
                          ...
                          dataKey="cpu" 
                          stroke="#2196F3" 
                          fill={`url(#cpuGradient-${server.id})`}
                          name="CPU"
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      ...
                      >
                        ...
                        <Area 
                          ...
                          stroke="#4CAF50" 
                          fill={`url(#memoryGradient-${server.id})`}
                          name="Memory"
                          isAnimationActive={false}
                        />
                        {refAreaLeft && refAreaRight ? (
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      ...
                      >
                        ...
                        <Area 
                          ...
                          stroke="#FF9800" 
                          fill={`url(#diskGradient-${server.id})`}
                          name="Disk"
                          isAnimationActive={false}
                        />
                        {refAreaLeft && refAreaRight ? (
                  ...
                  <Grid item xs={12} md={6}>
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      ...
                      >
                        <defs>
                          <linearGradient id={`uptimeGradient-${server.id}`} x1="0" y1="0" x2="0" y2="1">
                        ...
                        <Area 
                          ...
                          dataKey="uptime" 
                          stroke="#00BCD4" 
                          fill={`url(#uptimeGradient-${server.id})`}
                          name="Uptime"
                          isAnimationActive={false}
                        />
                        {refAreaLeft && refAreaRight ? (
                ...
                <Alert severity="info">
                  No metrics data available for this server.
                </Alert>
              )}
            </Paper>
          ))}
...
export default LinuxServerMonitoring;
