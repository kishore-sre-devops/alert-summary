...
const WindowsServerMonitoring = () => {
  ...
  const loadServers = async () => {
    ...
    try {
      ...
      // Filter only Windows servers
      const windowsServers = data.filter(server => 
        server.os && server.os.toLowerCase().includes('windows')
      );

      // Add calculated status
      const enhancedServers = windowsServers.map(server => {
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <ComputerIcon sx={{ fontSize: 40, color: '#2196F3' }} />
      ...
      {loading ? (
        ...
        <Alert severity="info">
          No Windows servers found in <strong>{environment.toUpperCase()}</strong> environment.
          {environment === 'uat' && (
            <Box sx={{ mt: 1 }}>
...
export default WindowsServerMonitoring;
