...
def run_db_audit():
    try:
        ...
        cur.execute("SELECT name, host, port FROM database_config WHERE db_type='mysql'")
        rows = cur.fetchall()
        ...
        cur.execute("SELECT name, host, port FROM database_config WHERE db_type='elasticsearch'")
        rows = cur.fetchall()
